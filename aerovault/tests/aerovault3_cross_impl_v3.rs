...
fn fixture_tree() -> Vec<(String, Vec<u8>)> {
    ...
    vec![
        ("readme.txt".to_string(), b"AeroVault v3 cross-impl fixture\n".to_vec()),
...
fn golden_is_byte_reproducible() {
    ...
    assert_eq!(a, b, "deterministic build must be byte-reproducible run to run");
...
fn crate_opens_and_extracts_golden() {
    ...
    if !gp.exists() {
        ...
        eprintln!("golden not present yet ({}); skipping read-path check", gp.display());
        return;
    }
    let tmp = tempfile::tempdir().unwrap();
    ...
    for (rel, bytes) in fixture_tree() {
        let got = std::fs::read(out.join(&rel)).unwrap_or_else(|_| panic!("missing extracted {rel}"));
        assert_eq!(got, bytes, "extracted {rel} is not byte-identical");
    }
    // The explicit empty directory survived.
    assert!(out.join("emptydir").is_dir(), "empty directory not extracted");
}
